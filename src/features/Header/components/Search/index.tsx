...
export default function Search({
  ...
  const { data, isLoading } = useGetGlobalSearch(searchParam);
  ...
  function getSearchItemDetails(item: ListItemType) {
    ...
    navigate(`/community/category-posts/replies?threadId=${item?.id}`, {
                ...
                <div className="divide-y max-h-96 overflow-y-scroll">
                  {searchedList?.map((item: ListItemType, index: number) => {
                    return (
                      <p
                        key={`${index - item?.id}`}
                        className="p-4 pl-10 text-slate-600 hover:bg-sky-50 truncate cursor-pointer"
                ...
                <div className="h-72 flex flex-col gap-3 items-center justify-center">
                  ...
                  >
                    ...
                    <rect
                      ...
                      fill="#fff"
                    />
                    <rect width="643.86163" height="27.3536" fill="#068fff" />
